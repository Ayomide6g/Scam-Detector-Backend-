...
export default async function handler(req, res) {
  ...
  // Now async because I added WHOIS
  const result = await analyzeMessage(text);
  return res.status(200).json(result);
}

// ===== COMPANY DATABASE - I'LL ADD 500+ HERE =====
const COMPANY_REGISTRY = [
  ...
  { name: 'Apple', industry: 'Tech', domains: ['apple.com', 'icloud.com'], ussd: [], never_asks_for: ['apple id via link'], official_channels: 'Settings on your device' },
  { name: 'Binance', industry: 'Crypto', domains: ['binance.com'], ussd: [], never_asks_for: ['seed phrase', '12 words'], official_channels: 'Binance app' },
  //I'LL Add 494 more companies here
];
...
const HIGH_RISK_KEYWORDS = ['otp', 'pin', 'password', 'cvv', 'bank account', 'verify now', 'account suspended', 'claim prize', 'send money', 'free bitcoin', 'double your money', 'mining pool', 'investment opportunity', 'passport photo', 'utility bill', 'id card', 'selfie with id', 'account number', 'routing number', 'iban', '2fa code'];
...
async function analyzeMessage(text) {
  ...
  // 1. Extract URLs - KEEPING REGEX
  const urlRegex = /(https?:\/\/[^\s]+)|(www\.[^\s]+)|([a-z0-9-]+\.[a-z]{2,})/gi;
  const urls = text.match(urlRegex) || [];

  // 2. Define keyword lists - MERGE MINE + NEW ONES
  ...
  if (urls.length > 0) {
    for (const url of urls) {
      ...
      // NEW: Domain age check
      try {
        const whoisData = await whois(parsed.domain || cleanUrl);
        if (whoisData.creationDate) {
          const ageDays = (Date.now() - new Date(whoisData.creationDate).getTime()) / (1000 * 60 * 60 * 24);
          if (ageDays < 30) {
            score += 50; reasons.push(`Domain ${cleanUrl} is only ${Math.floor(ageDays)} days old`);
          } else if (ageDays < 180) {
            score += 20; reasons.push(`Domain ${cleanUrl} is ${Math.floor(ageDays)} days old`);
          }
        }
      } catch (e) { /* WHOIS failed, skip */ }

      // NEW: Typosquat check
      if (detectedCompany) {
        for (const officialDomain of detectedCompany.domains) {
          const distance = leven(parsed.domain || cleanUrl, officialDomain);
          if (distance > 0 && distance <= 2) {
            score += 45; reasons.push(`Possible typosquat: ${cleanUrl} vs ${officialDomain}`);
          }
          if ((parsed.domain || cleanUrl) === officialDomain) {
            score -= 30; reasons.push(`Matches official ${detectedCompany.name} domain`);
          }
        }
      }
    }
  ...
  CRITICAL_KEYWORDS.forEach(keyword => {
    if (lowerText.includes(keyword)) {
      score += 40; reasons.push(`Critical data request: "${keyword}"`);
    }
  });
  HIGH_RISK_KEYWORDS.forEach(keyword => {
    if (lowerText.includes(keyword)) {
      score += 35; reasons.push(`High-risk phrase detected: "${keyword}"`);
    }
  });
  SUSPICIOUS_KEYWORDS.forEach(keyword => {
    if (lowerText.includes(keyword)) {
      score += 15; reasons.push(`Suspicious phrase detected: "${keyword}"`);
    }
  });
  ...
  if (detectedCompany) {
    for (const rule of detectedCompany.never_asks_for) {
      if (lowerText.includes(rule.split(' ')[0])) {
        score += 35; reasons.push(`${detectedCompany.name} never asks for "${rule}" via messages`);
      }
    }
  }
